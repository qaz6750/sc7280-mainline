...
static void dsi_update_dsc_timing(struct msm_dsi_host *msm_host, bool is_cmd_mode)
{
	...
	/*
	 * Each packet carries one or more complete slices. The packet count is
	 * therefore the number of slices on the interface divided by the number
	 * of slices carried in each packet.
	 */
...
{
	struct msm_dsi_host *msm_host = to_msm_dsi_host(host);
	unsigned int slice_per_pkt = 0;
	int ret;

	if (dsi->lanes > msm_host->num_data_lanes)
		return -EINVAL;
	if (dsi->dsc) {
		slice_per_pkt = dsi->dsc_slice_per_pkt ?: 1;

		if (slice_per_pkt > dsi->dsc->slice_count ||
		    dsi->dsc->slice_count % slice_per_pkt)
			return -EINVAL;
	}
	...
	msm_host->dsc_slice_per_pkt = slice_per_pkt;
